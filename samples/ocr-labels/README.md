...
| `synth_packaged-water.png` | 1 L packaged drinking water with `IS 14543` printed on its own line | standard **MATCHED → IS 14543:2016**; product confirmed; printed-IS-number check **PASS**; overall REVIEW (2 verified areas not checkable). `AQUA SPRING` is an UNCERTAIN brand, never the product name; OCR's `care@ clearflow.example` is normalized with the original kept |
| `synth_hallmark-closeup.png` | jewellery hallmark close-up: `BIS`, `22K916`, `HUID: K7M2Q9` (fictional HUID) — use as a **hallmark inspection** (Hallmarking page) | potential HUID **K7M2Q9 detected, NOT_VERIFIED**; purity 22K / 916 read at low OCR confidence → REVIEW; BIS logo and HUID authenticity NOT_SUPPORTED; Legal Metrology not applied; reported as needing external verification |
